...
fun AsymmetricCipherKeyPair.getPublicKey(): ByteArray {
    return (public as ECPublicKeyParameters).toByteArray()
}

fun AsymmetricCipherKeyPair.getPrivateKey(): ByteArray {
    return (private as ECPrivateKeyParameters).toByteArray()
}

fun AsymmetricCipherKeyPair.toKeyPair(): KeyPair {
//    val pkcs8Encoded: ByteArray = PrivateKeyInfoFactory.createPrivateKeyInfo(private).getEncoded()
//    val pkcs8KeySpec = PKCS8EncodedKeySpec(pkcs8Encoded)
//    val spkiEncoded = SubjectPublicKeyInfoFactory.createSubjectPublicKeyInfo(public).getEncoded()
//    val spkiKeySpec = X509EncodedKeySpec(spkiEncoded)
//    val keyFac: KeyFactory = KeyFactory.getInstance("EC")
